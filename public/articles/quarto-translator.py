...
def modify_html(file_path):
    ...
    for h1 in soup.find_all('h1'):
        h1['class'] = "font-heading text-4xl sm:text-6xl mb-12 mt-8 font-normal text-center"
    title_header = soup.find(id="title-block-header")
    title_header['class'] = "prose lg:prose-lg sm:text-center font-normal"
    for h2 in soup.find_all('h2'):
        h2['class'] = "text-2xl sm:text-3xl mb-4 mt-8 font-bold text-pacific-blue-600"
    for h3 in soup.find_all('h3'):
        h3['class'] = "text-lg sm:text-2xl mb-4 mt-8 font-bold text-pacific-blue-900"
    for h4 in soup.find_all('h4'):
        h4['class'] = "text-base sm:text-xl mb-4 mt-8 font-bold text-pacific-blue-900"
    ...
    for source_code in soup.find_all(class_="sourceCode"):
        if source_code.parent.name == "div" and not source_code.find_parent(class_="cell"):
            source_code['class'] = " ".join(source_code['class']) + "max-w-screen-lg text-sm lg:text-base mb-8 mt-4 p-2 shadow-gray-900 shadow-punk border border-gray-900"

    # 8. Ajouter des classes aux éléments avec la classe "cell"
    for cell in soup.find_all(class_="cell"):
        cell['class'] = " ".join(cell['class'])  + "max-w-screen-lg text-sm lg:text-base mb-8 mt-4 p-2 shadow-gray-900 shadow-punk border border-gray-900"
    ...
    if main:
        for p in main.find_all('p'):  # Ne trouve que les <p> directement dans <main>
            p['class'] = "prose lg:prose-lg prose-stone max-w-[40em]  mb-4"
            for code in p.find_all('code'):
                code['class'] = "p-1 text-pink-600 shadow-inner"
        for ul in main.find_all('ul'):
            ul['class'] = "prose prose-stone list-disc pl-8 lg:prose-lg"
            print(ul)
            for code in ul.find_all('code'):
                code['class'] = "p-1 text-pink-600 shadow-inner"
        for ol in main.find_all('ol'):
            ol['class'] = "prose prose-stone list-decimal pl-8 lg:prose-lg"
    ...
    for p in soup.find_all("p", string=lambda text: text and text.startswith('|') and text.endswith('|')):
        p["class"] = "prose lg:prose-lg text-lg !mb-16 text-gray-900 text-center"
        p["role"] = "doc-subtitle"
        p.string = '“' + p.text.strip('|').strip() + '”'

        or_tag = soup.new_tag("p")
        or_tag["class"] = 'prose lg:prose-lg text-xl text-gray-700 !mb-6 !-mt-6 text-gray-900 text-center'
        or_tag.string = "— OR —"
